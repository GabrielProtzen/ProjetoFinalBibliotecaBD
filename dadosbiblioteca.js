...
const resUsuarios = db.usuarios.insertMany(usuarios);
const usuariosIds = Object.values(resUsuarios.insertedIds);
...
const resLivros = db.livros.insertMany(livros);
const livrosIds = Object.values(resLivros.insertedIds);
...
for(let i = 0; i < 1000; i++){
    ...
    emprestimos.push({
        ...
        livro_id:
            livrosIds[Math.floor(Math.random()*livrosIds.length)],

        dataEmprestimo,

        dataPrevistaDevolucao:
            new Date(dataEmprestimo.getTime() + 15*24*60*60*1000),
